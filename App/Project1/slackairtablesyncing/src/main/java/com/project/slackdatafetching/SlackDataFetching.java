...
public class SlackDataFetching {
	...
	private static List<Conversation> fetchChannels(MethodsClient methods) {
		try {
			ConversationsListRequest request = ConversationsListRequest.builder().token(SLACK_TOKEN).build();
	...
	private static List<User> fetchUsers(MethodsClient methods) {
		try {
			UsersListRequest request = UsersListRequest.builder().token(SLACK_TOKEN).build();
	...
	private static void writeChannelsToCSV(List<Conversation> channels, String fileName) {
		try (BufferedWriter writer = new BufferedWriter(new FileWriter(fileName))) {
			// Write CSVV header
			writer.write(
					"Channel Name,Channel ID,Topic,Description,Creator ID,Channel Create Date,Privacy,Members,Status");
			writer.newLine();

			// Write channel dât
			for (Conversation channel : channels) {
				writer.write(String.format("%s,%s,%s,%s,%s,%s,%s,%d,%s", channel.getName(), channel.getId(),
						getFieldValue(channel.getTopic().getValue()), getFieldValue(channel.getPurpose().getValue()),
						getFieldValue(channel.getCreator()), formatDate(channel.getCreated()),
						getFieldValue(channel.isPrivate() ? "Private" : "Public"), channel.getNumOfMembers(),
	...
	private static void writeUsersToCSV(List<User> users, String fileName) {
		try (BufferedWriter writer = new BufferedWriter(new FileWriter(fileName))) {
			// Write CSV header
			writer.write(
					"User Name,User ID,Email,Display Name,Full Name,Status,Role,User Created Date,Status Change Date");
			writer.newLine();

			// Write user data
			for (User user : users) {
				writer.write(String.format("%s,%s,%s,%s,%s,%s,%s,%s,%s", getFieldValue(user.getName()), user.getId(),
						user.getProfile().getEmail(), user.getProfile().getDisplayName(),
						user.getProfile().getRealName(), user.getProfile().getStatusText(),
						user.isOwner() ? "Owner" : "Member", formatDate(user.getUpdated()),
